...
- **Internal Tools First**: Use internal tools for reading and writing files instead of OS commands (e.g., do not use `cat`, `type`, `grep`, or `echo >` for file operations).
...
- **Tools allowance**: Only allow to use internal read/write tools and `bun` command.

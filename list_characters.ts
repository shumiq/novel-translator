...
function getEnglishAlias(aliases: string[] | undefined): string {
  ...
  // Find the first English alias (contains Latin characters)
  const englishAlias = aliases.find((alias) => /\p{Script=Latin}/u.test(alias));
...
if (unknownCharacters.length > 0) {
  ...
  unknownCharacters.forEach((char) => {
    output += `- ${char.alias} (Unknown gender)\t${char.description}\n`;
  });
}
